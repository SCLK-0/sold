...
this.worldHeight = 1600;  // bigger dungeon
  ...
  this.boss.scale = 20;
    ...
    // Hide touch controls completely on desktop
    const mobileControls = document.getElementById('mobileControls');
    if (mobileControls) {
...
const shootJoystick = document.getElementById('shootJoystick');
  ...
  e.preventDefault();
  const touch = e.touches[0];
  const rect = shootControls.getBoundingClientRect();
  this.touchControls.shootArea.active = true;
  this.touchControls.shootArea.centerX = rect.left + rect.width / 2;
  this.touchControls.shootArea.centerY = rect.top + rect.height / 2;
  this.updateShootJoystick(touch);
});

shootControls.addEventListener('touchmove', (e) => {
  e.preventDefault();
  if (this.touchControls.shootArea.active) {
    const touch = e.touches[0];
    this.updateShootJoystick(touch);
  }
});
...
shootControls.addEventListener('touchend', (e) => {
  ...
  this.touchControls.shootArea.x = 0;
  this.touchControls.shootArea.y = 0;
  ...
  const centerX = this.touchControls.shootArea.centerX;
  const centerY = this.touchControls.shootArea.centerY;
  ...
  const shootJoystick = document.getElementById('shootJoystick');
  shootJoystick.style.transform = `translate(${deltaX - 50}%, ${deltaY - 50}%)`;

  // normalized aim vector
  this.touchControls.shootArea.x = deltaX / maxDistance;
  this.touchControls.shootArea.y = deltaY / maxDistance;

  // shoot if joystick is moved
  const threshold = 0.3;
  const isAiming =
    Math.abs(this.touchControls.shootArea.x) > threshold ||
    Math.abs(this.touchControls.shootArea.y) > threshold;
  this.mouse.down = isAiming;

  if (isAiming) {
    const angle = Math.atan2(
      this.touchControls.shootArea.y,
      this.touchControls.shootArea.x
    );
    this.mouse.x = this.player.x + Math.cos(angle) * 100;
    this.mouse.y = this.player.y + Math.sin(angle) * 100;
  }
}
...
if (this.mouse.down && this.player.canShoot()) {
    // Always shoot toward where the player is aiming (mouse or touch)
...
const zoom = this.isMobile ? 1.5 : 1.0;
...
drawDungeonWalls() {
    ...
    // Dungeon brick walls
    const wallGradient = ctx.createLinearGradient(0, 0, 0, 50);
    ...
    ctx.fillRect(0, 760, 1000, 40);
    // Left & right walls
    ctx.fillRect(0, 0, 40, 800);
    ctx.fillRect(960, 0, 40, 800);
...
drawDungeonLighting() {
    ...
    drawGameStateMessages() {
        ...
        if (this.gameState === 'bossDefeated') {
            ...
            this.ctx.font = '16px Courier New';
            this.ctx.fillText('Try again', this.width / 2, this.height / 2 + 40);
        }
